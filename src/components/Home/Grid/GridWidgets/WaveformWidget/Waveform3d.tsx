...
const Waveform3d = (): JSX.Element => {
  ...
  const [activeInd, setActiveInd] = useState(
    tracks.indexOf(tracks.filter((t) => t.title === currentTrack.title)[0])
  );
  ...
  const [activeInfoDisplayLocal, setActiveInfoDisplayLocal] =
    useState(infoDisplayMode);
...
const Geo = ({ track }: { track: Track; active: number }): JSX.Element => {
  const audioElem = useRef<HTMLAudioElement>();
  const videoElem = useRef<HTMLMediaElement>();
  ...
  useEffect(() => {
    ...
    videoElem.current = document.getElementById(
      "recital_video"
    ) as HTMLMediaElement;
  }, [track.title]);
  ...
  useFrame((state) => {
    ...
    if (track.category === "recital") {
      if (videoElem.current && !videoElem.current.paused && groupRef.current) {
        groupRef.current.position.x -= 0.1;
      }
    }
